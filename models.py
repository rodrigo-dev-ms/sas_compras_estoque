...
class User(Base):
    ...
    Atributos:
        ...
        role     (str)  : Perfil de acesso. Ex: 'admin', 'solicitante', 'estoque'.
    """
    ...
    role: Mapped[str] = Column(String(50), nullable=False, default="solicitante")

    # Relacionamento reverso: um usuário pode ter várias solicitações
...
class Solicitacao(Base):
    ...
    Atributos:
        ...
        status         (str)        : Estado da solicitação. Default: 'ABERTA'.
        data_criacao   (date)       : Data de abertura da solicitação.
        solicitante_id (int)        : FK para users.id.
        ...
        materiais      (list | None): Lista de materiais em formato JSON.
                                      Estrutura esperada:
                                      [{"material": str, "descricao": str,
                                        "un": str, "qtd": int}, ...]
    ...
    id: Mapped[int] = Column(Integer, primary_key=True, index=True)
    status: Mapped[str] = Column(String(50), nullable=False, default="ABERTA")
    data_criacao: Mapped[date] = Column(Date, nullable=False, default=date.today)
    ...
    # Relacionamento: acessa o objeto User a partir da solicitação
